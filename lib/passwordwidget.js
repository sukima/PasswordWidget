...
function PasswordWidget(selector, options) {
  ...
  this.element_
    .assertTag('input')
    .addClass('pw-input')
    .proxyEvent('keyup', emitUpdateEvent.bind(this));
}
...
PasswordWidget.prototype.infoButton = function() {
  ...
  function onClick(evt) {
    ...
    if (this.hasListeners('showInfo')) {
      this.emit('showInfo', {content: defaults.locale.infoText});
    } else {
      DOMBuilder.showAlert(defaults.locale.infoText);
    }
    this.emit('action', {type: 'pw-info'});
    return false;
  }

  var button = DOMBuilder.button(defaults.locale.infoLabel)
...
PasswordWidget.prototype.showHideButton = function() {
  ...
  function setButtonState(isMasked) {
    if (isMasked) {
      button.domElement.innerHTML = defaults.locale.show[0];
      button.domElement.title     = defaults.locale.show[1];
    } else {
      button.domElement.innerHTML = defaults.locale.mask[0];
      button.domElement.title     = defaults.locale.mask[1];
    }
  }
  ...
  function onClick(evt) {
    ...
    this.emit('action', {type: 'pw-show-mask', isMasked: this.isMasked});
    return false;
  }
